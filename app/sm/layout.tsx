...
export default function SMLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-[430px] flex-col bg-white">
      {/* Topbar */}
      <header className="sticky top-0 z-30 flex h-[60px] items-center justify-between border-b border-gray-200 bg-white px-4">
      ...
      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-1/2 z-40 flex w-full max-w-[430px] -translate-x-1/2 border-t border-gray-200 bg-white">
        {bottomNav.map((item) => {
            ...
            >
              <div className="relative">
                ...
                {item.label === "Notifications" && unreadCount > 0 && (
                  <span className="absolute -right-1.5 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-error-600 text-[9px] font-bold text-white">
